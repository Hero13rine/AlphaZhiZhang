...
class TrainPipeline():
    ...
    def collect_selfplay_data(self, data):
        ...
        print('样本池容量 {}'.format(len(self.data_buffer)))
    ...
    def run(self, lock):
        ...
        try:
            if len(self.data_buffer) > self.batch_size:
                ...
                # self.policy_value_net.save_model('./current2_policy.model')
                # lock.release()
...
def train(q, lock):
    ...
    while True:
        ...
        if newGameNum != 0 and get_times >= 0:
            lock.acquire()
            training_pipeline.run(lock)
            newGameNum -= 1
            lock.release()
...
if __name__ == '__main__':
    ...
    # p3 = mp.Process(target=collect_data, args=(q, lock,))
    # p4 = mp.Process(target=collect_data, args=(q, lock,))
